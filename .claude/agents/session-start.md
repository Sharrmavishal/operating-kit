---
name: session-start
description: Run at the start of every work session. Reads the canonical state doc, verifies live state, and prints a concise briefing: current versions, deployed revision, open issues, and what's next. Prevents stale-state mistakes.
tools: Read, Bash
---
...
- Live check: [actual value] ✅/⚠️ (match / MISMATCH with state doc)
...
If the live check disagrees with the state doc, **flag it loudly**: "⚠️ MISMATCH. State doc says
