...
Rows ≠ entities — use `COUNT(DISTINCT)` when a key can repeat. A mean hides skew — look at
the distribution, not just the average. Most "alarming numbers" are one of these two
